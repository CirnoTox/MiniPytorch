...
class Transpose(TensorOp):
    ...
    def compute(self, a):
        ...
        if (self.axes is not None):
            tmpAxes[self.axes[0]], tmpAxes[self.axes[1]
                                           ] = tmpAxes[self.axes[1]], tmpAxes[self.axes[0]]
...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if len(shape_in) != len(shape_out):
            out_grad = summation(out_grad, tuple(
                i for i in range(len(shape_out) - len(shape_in))))
        axes = []
...
class MatMul(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if len(lhs_grad.shape) != len(lhs.shape):
            lhs_grad = summation(lhs_grad, axes=tuple(
                i for i in range(len(lhs_grad.shape) - len(lhs.shape))))
        if len(rhs_grad.shape) != len(rhs.shape):
            rhs_grad = summation(rhs_grad, axes=tuple(
                i for i in range(len(rhs_grad.shape) - len(rhs.shape))))
